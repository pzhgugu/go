...
<div class="goods-gallery add-step2">
  <a class='sample_demo' id="select_s" href="${S_URL}/se/goods/addstep/two/imagesdesc"
  style="display:none;">
    提交
  </a>
  <div class="nav">
...
<c:if test="${fn:length(P_ALBUM_PICLIST)>0}">
  ...
  <c:forEach items="${P_ALBUM_PICLIST}" var="pic">
    <li onclick="insert_editor('${S_URL}/att/download/${pic.apicCover}');">
   ...
   <fis:block url="shop:widget/tpl/pagination.html.jsp" >
       ...
       <fis:param name="url" value="${S_URL}/se/goods/addstep/two/imagesdesc"/>
     </fis:block> 
</c:if>
<c:if test="${fn:length(P_ALBUM_PICLIST)==0}">
...
$(function() {
    ...
    $('#jumpMenu').change(function() {
      $('#select_s').attr('href', $('#select_s').attr('href') + "?class_id=" + $('#jumpMenu').val());
