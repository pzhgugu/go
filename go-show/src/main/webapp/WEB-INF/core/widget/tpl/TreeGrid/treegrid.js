...
var GPC = {
	url:{
		...
		queryUrl:GLOBAL.S.URL+GLOBAL.P.MODULES+"/a/queryPage/"+GLOBAL.P.CLASSNAME+"?_key="+GLOBAL.P.TREEONENAME+".id",
		saveUrl:GLOBAL.S.URL+GLOBAL.P.MODULES+"/a/save/"+GLOBAL.P.CLASSNAME+"?_key="+GLOBAL.P.TREEONENAME+".id",
		detailQueryUrl:GLOBAL.S.URL+GLOBAL.P.MODULES+"/tpl/DetailQuery/"+GLOBAL.P.CLASSNAME+"/d?_key="+GLOBAL.P.TREEONENAME+".id",
...
GPW.grid = {
	...
	/* onKeyPressed : function(code,ctrl,shift){
		if(code==67&&ctrl){
			if (!this.mainGrid._selectionArea) return alert("You need to select a block area in grid first");
			this.mainGrid.setCSVDelimiter("\t")
			this.mainGrid.copyBlockToClipboard()
		}
		return true;
	}, */
	init : function() {
		...
		/* this.mainGrid.enableAlterCss("even","uneven");
		this.mainGrid.attachEvent("onBeforeSelect",function(){ return false; });
		this.mainGrid.enableBlockSelection();
		this.mainGrid.attachEvent("onKeyPress",GPW.grid.onKeyPressed); */
		...
		this.gridDataProcessor = new dataProcessor(GPC.url.updateUrl+"&_value="+selectId); //lock feed url
