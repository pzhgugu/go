...
GPW.tabbar={
		...
		initSubList:function(){
			this.subListTabbar=GPW.layout.tabbarLayout.attachTabbar({
				tabs: [
						{ id: "resourceRole", text: "角色授权", active: true},
						{ id: "PermissionURL", text: "认证链接"},
						{ id: "Permission", text: "认证类型"}
			...
			this.subListTabbar.attachEvent("onSelect", function(actvId, lastId){
				...
				var url ;
				if(actvId=="resourceRole"){
					url = GLOBAL.S.URL+GLOBAL.P.MODULES+"/"+actvId+"?_value="+id;
				}else{
					url = GPC.url.loadActiveTabUrl+"/"+actvId+"/d?_key=resources.id&_value="+id;
				}
