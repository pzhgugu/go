...
public class EntityFieldsCategoryExecute extends AbstractExecuteXml implements IExecuteXml{

	@Override
	public Class getClazz() {
		return EntityFieldsCategory.class;
	}
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<EntityFieldsCategory> xmlEntityList = (List<EntityFieldsCategory>) o;
		Collection<EntityFieldsCategory> dataBaseList = baseService.findAll(EntityFieldsCategory.class);
		for (EntityFieldsCategory entity : xmlEntityList) {
			EntityFieldsCategory xmlEntity =new EntityFieldsCategory();
			try {
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (EntityFieldsCategory dataBaseEntity : dataBaseList) {
				//数据库节点名称
				...
				if(dataBaseName.equals(xmlName)){
					...
					if(dataBaseParentName.equals(parentXmlName)){
						...
						if (dataBaseEntity.getVersionPublish() != xmlEntity.getVersion()) {
							EntityFieldsCategory uEntity = getUpdateEntity(dataBaseEntity, xmlEntity);
							baseService.save(uEntity);
						}
						isSave=false;
						break;
					}
	...
	private EntityFieldsCategory getUpdateEntity(EntityFieldsCategory databaseEntity, EntityFieldsCategory entity) {
