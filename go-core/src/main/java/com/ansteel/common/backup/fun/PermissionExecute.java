...
public class PermissionExecute extends AbstractExecuteXml implements IExecuteXml{
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<Permission> xmlEntityList = (List<Permission>) o;
		Collection<Permission> dataBaseList = baseService.findAll(Permission.class);
		for (Permission entity : xmlEntityList) {
			Permission xmlEntity =new Permission();
			try {
			...
			String xmlName = xmlEntity.getAlias();
			//此节点只有ID
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (Permission dataBaseEntity : dataBaseList) {
				//数据库节点名称
				String dataBaseName = dataBaseEntity.getAlias();
	...
	private Permission getUpdateEntity(Permission databaseEntity, Permission entity) {
