...
public class EntityFieldsGridExecute extends AbstractExecuteXml implements
		IExecuteXml {

	@Override
	public Class getClazz() {
		return EntityFieldsGrid.class;
	}

	@Override
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {

		List<EntityFieldsGrid> xmlEntityList = (List<EntityFieldsGrid>) o;

		Map<String, EntityFieldsCategory> parentDataBase = this
				.getParentDataBase(xmlMap);
	...
	private Map<String, EntityFieldsCategory> getParentDataBase(
		...
		// 得到所有父类
		List<EntityFieldsCategory> xmlParentList = (List<EntityFieldsCategory>) xmlMap
				.get(EntityFieldsCategoryExecute.class);
		// 得到所有顶层
		List<Models> xmlTopList = (List<Models>) xmlMap
				.get(ModelsExecute.class);
		...
		for (Entry<EntityFieldsCategory, Models> entry : xmlTopNameToParent
				.entrySet()) {
			...
			if (dataBaseParent.containsKey(name)) {
				Collection<EntityFieldsCategory> dataBaseParentList = dataBaseParent
						.get(name);
				boolean is = false;
	...
	private void save(Map<String, EntityFieldsCategory> parentDataBase,
			List<EntityFieldsGrid> xmlEntityList) {

		for (EntityFieldsGrid entity : xmlEntityList) {
			EntityFieldsGrid xmlEntity = new EntityFieldsGrid();
			try {
			...
			if (parentDataBase.containsKey(xmlParentId)) {
				EntityFieldsCategory parentDataBaseEntity = parentDataBase
						.get(xmlParentId);
				xmlEntity.setFieldsCategory(parentDataBaseEntity);
				// 确认xml中是否有数据库中的实体
				boolean isSave = true;

				Collection<EntityFieldsGrid> dataBaseEntityList = parentDataBaseEntity
						.getFieldsGrid();
				String xmlName = xmlEntity.getName();
				for (EntityFieldsGrid dataBaseEntity : dataBaseEntityList) {

					String dataBaseName = dataBaseEntity.getName();
					if (xmlName.equals(dataBaseName)) {
						if (dataBaseEntity.getVersionPublish() != xmlEntity
								.getVersion()) {
							baseService.save(getUpdateEntity(dataBaseEntity,
									xmlEntity));
						}
						isSave = false;
						break;
	...
	private EntityFieldsGrid getUpdateEntity(EntityFieldsGrid databaseEntity,
			EntityFieldsGrid entity) {
