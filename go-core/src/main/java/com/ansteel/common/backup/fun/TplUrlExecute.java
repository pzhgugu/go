...
public class TplUrlExecute extends AbstractExecuteXml implements IExecuteXml{
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<TplUrl> xmlEntityList = (List<TplUrl>) o;
		Collection<TplUrl> dataBaseList = baseService.findAll(TplUrl.class);
		for (TplUrl entity : xmlEntityList) {
			TplUrl xmlEntity =new TplUrl();
			try {
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (TplUrl dataBaseEntity : dataBaseList) {
				//数据库节点名称
	...
	private TplUrl getUpdateEntity(TplUrl databaseEntity, TplUrl entity) {
