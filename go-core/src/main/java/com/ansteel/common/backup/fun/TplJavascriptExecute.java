...
public class TplJavascriptExecute extends AbstractExecuteXml implements IExecuteXml{

	@Override
	public Class getClazz() {
		return TplJavascript.class;
	}
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<TplJavascript> xmlEntityList = (List<TplJavascript>) o;
		Collection<TplJavascript> dataBaseList = baseService.findAll(TplJavascript.class);
		for (TplJavascript entity : xmlEntityList) {
			TplJavascript xmlEntity =new TplJavascript();
			try {
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (TplJavascript dataBaseEntity : dataBaseList) {
				//数据库节点名称
	...
	private TplJavascript getUpdateEntity(TplJavascript databaseEntity, TplJavascript entity) {
