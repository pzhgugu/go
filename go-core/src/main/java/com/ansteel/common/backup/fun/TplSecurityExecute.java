...
public class TplSecurityExecute  extends AbstractExecuteXml implements IExecuteXml{

	@Override
	public Class getClazz() {
		return TplSecurity.class;
	}
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<TplSecurity> xmlEntityList = (List<TplSecurity>) o;
		Collection<TplSecurity> dataBaseList = baseService.findAll(TplSecurity.class);
		for (TplSecurity entity : xmlEntityList) {
			TplSecurity xmlEntity =new TplSecurity();
			try {
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (TplSecurity dataBaseEntity : dataBaseList) {
				//数据库节点名称
	...
	private TplSecurity getUpdateEntity(TplSecurity databaseEntity, TplSecurity entity) {
