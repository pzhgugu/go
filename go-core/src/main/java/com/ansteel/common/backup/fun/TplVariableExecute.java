...
public class TplVariableExecute  extends AbstractExecuteXml implements IExecuteXml{

	@Override
	public Class getClazz() {
		return TplVariable.class;
	}
	...
	public void importSave(Map<Class, Object> xmlMap, Object o, int type) {
		...
		List<TplVariable> xmlEntityList = (List<TplVariable>) o;
		Collection<TplVariable> dataBaseList = baseService.findAll(TplVariable.class);
		for (TplVariable entity : xmlEntityList) {
			TplVariable xmlEntity =new TplVariable();
			try {
			...
			//确认xml中是否有数据库中的实体
			boolean isSave = true;
			for (TplVariable dataBaseEntity : dataBaseList) {
				//数据库节点名称
	...
	private TplVariable getUpdateEntity(TplVariable databaseEntity, TplVariable entity) {
